...
class QwenSFTTrainer(Trainer):
    ...
    def evaluation_step(self, model, inputs):
        """
        Custom evaluation step to compute eval_loss
        """
        model.eval()
        with torch.no_grad():
            loss = model(**inputs).loss
        return {"eval_loss": loss.detach().float()}

    def compute_metrics(self, eval_preds):
        """
        Compute metrics for evaluation
        """
        # For language modeling, we typically just return the loss
        # The eval_preds.predictions contains the loss values from evaluation_step
        if hasattr(eval_preds, 'predictions') and eval_preds.predictions is not None:
            # If predictions contain loss values, return them
            if isinstance(eval_preds.predictions, (list, tuple)) and len(eval_preds.predictions) > 0:
                eval_loss = float(eval_preds.predictions[0])
                return {"eval_loss": eval_loss}
        
        # If no predictions, try to get loss from label_ids
        if hasattr(eval_preds, 'label_ids') and eval_preds.label_ids is not None:
            if isinstance(eval_preds.label_ids, (list, tuple)) and len(eval_preds.label_ids) > 0:
                eval_loss = float(eval_preds.label_ids[0])
                return {"eval_loss": eval_loss}
        
        # Fallback: return empty dict but don't fail
