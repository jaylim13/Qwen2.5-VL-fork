#!/usr/bin/env python3
"""
Evaluation script for Qwen2.5-VL LoRA video action classification model.
Loads trained model and evaluates on test set with detailed metrics.
"""
...
def extract_prediction(response_text):
    ...
    valid_actions = ["walking", "sitting", "standing", "upstair", "downstair", "go upstair", "go downstair"]
...
def evaluate_single_sample(model, processor, sample, image_folder):
    ...
    try:
        ...
        # Use the same prompt as the 80% accuracy approach
        prompt = "Analyze this video clip and classify the action being performed. Choose ONLY ONE action from this list: walking, sitting, standing, upstair, downstair. Respond with just the action name, nothing else."
        ...
        # Generate response (same settings as 80% accuracy approach)
        with torch.no_grad():
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate trained LoRA model on test set")
    ...
    parser.add_argument("--output_dir", type=str, default="../../../../output/lora_video_action/eval_results",
    ...
    if args.use_base_model:
        output_dir = args.output_dir.replace("eval_results", "eval_results_base_model")
        print(f"📝 Using base model - results will be saved to: {output_dir}")
    elif args.use_merged_model:
        output_dir = args.output_dir.replace("eval_results", "eval_results_merged_model")
